function [assignmentlist, cost] = bipartite_matching(distMatrix)
%BIPARTITE MATCHING    Compute optimal assignment by Munkres algorithm
...
distMatrix = 100000*(distMatrix == 0) + distMatrix;
...
assignmentlist = zeros(length(assignment), 2);
for i = 1 : length(assignment)
    assignmentlist(i, 1) = i + nOfRows;
    assignmentlist(i, 2) = assignment(i);
end
